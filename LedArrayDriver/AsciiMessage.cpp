 ...
 *
 */
void AsciiMessage::buffer(unsigned char *dest) {
	for(unsigned short i = 0;i<strlen(_message);i++) {
		unsigned char letter = _message[i];
		unsigned char *matrix = ascii[letter-32];
		//
		// Each character has a number of columns
		//
		for (unsigned char column = 0; column < COLUMNS_PER_CHARACTER;column++) {
			unsigned char pattern = matrix[column];
			//
